...
subtitle: "Pass 2 Macrophages & Monocytes QC, DE, & Pseudotime"
author: "Kennedi Todd"
date: "05/11/2023"
output:
  html_document:
...
## Load data
```{r read_pass2_macrophages_monocytes_annotated}
# load
macrophages <- readRDS("../../rObjects/pass2_macrophages_monocytes_recluster_annotated.rds")

# reset levels
macrophages$sample <- factor(macrophages$sample,levels = sample_order)
macrophages$sample2 <- factor(macrophages$sample2,levels = sample_order2)
macrophages$sex <- factor(macrophages$sex,levels = sex_order)
macrophages$isoform <- factor(macrophages$isoform,levels = isoform_order)
```
...
## 3D UMAP
```{r 3D_umap}
embeddings <- macrophages@reductions$umap@cell.embeddings
embeddings <- cbind(embeddings, as.character(macrophages$annotated_clusters))
...
goi1 <- c("Tfrc","Mki67","Ly6c2","Ccr2","Lyz2","Mrc1","H2-Eb1","H2-Aa","Tyrobp",
...
```{r umap_annotated}
# Apoe isoform
umap1 <- DimPlot(object = macrophages, 
                 group.by = "annotated_clusters",
                 reduction = "umap",
                 split.by = "isoform",
                 repel = TRUE,
...
# sex
umap2 <- DimPlot(object = macrophages, 
                 group.by = "annotated_clusters",
                 reduction = "umap",
                 split.by = "sex",
                 repel = TRUE,
...
# sample
umap3 <- DimPlot(object = macrophages, 
                 group.by = "annotated_clusters",
                 reduction = "umap",
                 split.by = "sample2",
                 ncol = 2,
...
# phase
umap4 <- DimPlot(object = macrophages, 
                 group.by = "annotated_clusters",
                 reduction = "umap",
                 split.by = "phase",
                 cols = cluster_colors,
...
# mito.factor
umap5 <- DimPlot(object = macrophages, 
                 group.by = "annotated_clusters",
                 reduction = "umap",
...
```{r, echo=FALSE, eval=FALSE}
path <- "../../results/pass_2/macrophages_monocytes/clustering_QC/heatmap_"
...
VlnPlot(macrophages,
        ...
        split.by = "annotated_clusters",
        cols = cluster_colors)
FeaturePlot(object = macrophages, 
            features = "Ttr")
```
...
Idents(macrophages) <- macrophages$annotated_clusters
macrophages <- BuildClusterTree(object = macrophages,
                                     dims = 1:15,
                                     reorder = FALSE,
                                     reorder.numeric = FALSE)

tree <- macrophages@tools$BuildClusterTree
...
```{r,echo=FALSE,eval=FALSE}
p
path <- paste0("../../results/pass_2/macrophages_monocytes/clustering_QC/cluster_tree_annotated")
...
## Cells per sample
```{r}
data <- as.data.frame(table(macrophages$sample))
...
```{r,echo=FALSE,eval=FALSE}
ncells
path <- "../../results/pass_2/macrophages_monocytes/clustering_QC/cells_per_sample"
...
cell_types <- levels(macrophages$annotated_clusters)
...
for (i in cell_types) {
  print(i)
  cluster <- subset(macrophages, annotated_clusters == i)
...
# write table
write.table(female.df, "../../results/pass_2/macrophages_monocytes/DEGs/E4_F_vs_E3_F_DEGs.tsv", 
...
# write table
write.table(male.df, 
            "../../results/pass_2/macrophages_monocytes/DEGs/E4_M_vs_E3_M_DEGs.tsv", 
            sep = "\t",quote = FALSE, row.names = FALSE)
```
...
female.df <- read.table(
  "../../results/pass_2/macrophages_monocytes/DEGs/E4_F_vs_E3_F_DEGs.tsv",
  sep = "\t", header = TRUE)
male.df <- read.table(
  "../../results/pass_2/macrophages_monocytes/DEGs/E4_M_vs_E3_M_DEGs.tsv",
  sep = "\t", header = TRUE)
...
write.table(
  df, 
  "../../results/pass_2/macrophages_monocytes/DEGs/DEG_comparison_pvaladj_0.05.tsv",
  sep = "\t", quote = FALSE, row.names = FALSE)
```
...
# read tables
female.df <- read.table("../../results/pass_2/macrophages_monocytes/DEGs/E4_F_vs_E3_F_DEGs.tsv",
                         sep = "\t", header = TRUE)
male.df <- read.table("../../results/pass_2/macrophages_monocytes/DEGs/E4_M_vs_E3_M_DEGs.tsv",
                         sep = "\t", header = TRUE)
...
clusters <- levels(macrophages$annotated_clusters)
clusters <- clusters[2:5]
for (i in clusters) {
  ...
  i <- gsub("/","_",i)
  i <- gsub("-","_",i)
  pdf(paste0("../../results/pass_2/macrophages_monocytes/upset/upset_",
             tolower(i),".pdf"), height = 6, width = 8)
  print(upset_gene)
  dev.off()
}
```
...
all_clusters <- levels(macrophages$annotated_clusters)
all_clusters <- all_clusters[1:5]
...
for (i in variables) {
  ...
  if (i == "E4_F_vs_E3_F") {
    treatment_vs_control <- 
      read.delim("../../results/pass_2/macrophages_monocytes/DEGs/E4_F_vs_E3_F_DEGs.tsv",
                 sep = "\t")
  } else {
    treatment_vs_control <-
      read.delim("../../results/pass_2/macrophages_monocytes/DEGs/E4_M_vs_E3_M_DEGs.tsv",
                 sep = "\t")
  }
  ...
  for (j in all_clusters) {
    ...
    if(num != 0) {
      ...
      clus <- gsub("-","_",clus)
      if (i == "E4_F_vs_E3_F") {
        path <- paste0("../../results/pass_2/macrophages_monocytes/volcano/E4_F_vs_E3_F/",
                       tolower(clus),"_female_volcano")
      } else {
        path <- paste0("../../results/pass_2/macrophages_monocytes/volcano/E4_M_vs_E3_M/",
                       tolower(clus),"_male_volcano")      
      }
      print(p)
      saveToPDF(paste0(path,".pdf"), height = 8, width = 8)
      #print(p)
      dev.off()
...
cds <- as.cell_data_set(macrophages, default.reduction = "UMAP")
...
cds@clusters$UMAP$clusters <- macrophages$annotated_clusters
cds@int_colData@listData$reducedDims$UMAP <-macrophages@reductions$umap@cell.embeddings
```
...
starting.cells <- cds@colData$annotated_clusters == "Monocytes"
cds <- order_cells(cds, 
...
p1 + p2
path <- "../../results/pass_2/macrophages_monocytes/pseudotime/pseudotime.pdf"
...
p3
path <- "../../results/pass_2/macrophages_monocytes/pseudotime/pseudotime_split_sex.pdf"
saveToPDF(path, height = 4, width = 8)
dev.off()

p4
path <- "../../results/pass_2/macrophages_monocytes/pseudotime/pseudotime_split_isoform.pdf"
saveToPDF(path, height = 4, width = 8)
dev.off()

p5
path <- "../../results/pass_2/macrophages_monocytes/pseudotime/pseudotime_split_sex&isoform.pdf"
