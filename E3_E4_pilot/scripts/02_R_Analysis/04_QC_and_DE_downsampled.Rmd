...
subtitle: "Downsampled: QC and DE"
author: "Kennedi Todd"
date: "09/29/2024"
output:
  html_document:
...
mouse.downsample <- subset(x = mouse.annotated, downsample = 3000)
...
levels(mouse.downsample$annotated_clusters)[!levels(mouse.downsample$annotated_clusters) %in% unique(female.df$cluster)]
```
...
# print cell types that don't have DEGs
levels(mouse.downsample$annotated_clusters)[!levels(mouse.downsample$annotated_clusters) %in% unique(male.df$cluster)]
```
...
cell_types <- c(levels(mouse.downsample$annotated_clusters), "direction")
for (i in cell_types) {
  if (!i %in% colnames(df)) {
    print(i)
    df[[i]] <- 0
...
for (i in variables) {
  ...
  for (j in all_clusters) {
    ...
    if(num != 0) {
      ...
      pdf(paste0(path,".pdf"), height = 8, width = 8)
      print(p)
      dev.off()
...
for (i in clusters) {
  ...
  write.table(x = exclusive_intersection_df,
              file = paste0(out, "DEGs/upset/", tolower(i),"_upset.tsv"),
              sep = "\t",
              quote = FALSE,
              col.names = TRUE,
              row.names = FALSE)
