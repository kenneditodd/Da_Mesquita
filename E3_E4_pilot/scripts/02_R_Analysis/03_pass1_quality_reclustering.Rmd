...
cluster_colors <- c("firebrick1","blue","gold","orange","cyan","green","forestgreen",
                    "darkorchid1","red3","gray","deeppink","chocolate4","tan",
                    "steelblue","pink","black")
...
# Recluster group 1
Group 1 consists of Unknown1, Unknown 3, Schwann cells
...
Idents(mouse.annotated) <- "annotated_clusters"
myClusters <- c("Unknown 1", "Unknown 3", "Schwann cells")
keepCells <- mouse.annotated@meta.data[,"annotated_clusters"] %in% myClusters
keepCol <- c(4:14,24,25)
...
v2 <- VlnPlot(group1,
              ...
              flip = TRUE,
              cols = cluster_colors,
              features = c("Gfap","Tmem119","Tnr","Gad1","Gad2","Syt1","Mbp",
                           "Col1a1","Col1a2"))
v2
```
...
group1.annotated <- RenameIdents(object = group1,
                                ...
                                "1" = "Unknown",    
                                "2" = "Unknown",     
                                "3" = "Myelinating Schwann cells", # Mpz      
                                "4" = "Schwann cell precursors", # Sox10,Foxd3
                                "5" = "Unknown",
                                "6" = "Myelinating Schwann cells", # Mpz
                                "7" = "Unknown")
...
group1.annotated$annotated_clusters <- factor(group1.annotated$annotated_clusters,
                                              levels = c("Myelinating Schwann cells",
...
# set colors
cluster_colors <- c("red","green","gold")
...
v3 <- VlnPlot(group1.annotated,
              group.by = "annotated_clusters",
              split.by = "annotated_clusters",
              stack = TRUE,
              flip = TRUE,
              cols = cluster_colors,
              features = c("Mpz","S100b","Foxd3","Mbp","Gfap","Egr2",
                           "Acta2","Myh11","Flt1","C1qb","Col1a1"))
v3

v4 <- VlnPlot(group1.annotated,
              group.by = "annotated_clusters",
              split.by = "annotated_clusters",
              stack = TRUE,
              flip = TRUE,
              ...
              features = c("nCount_RNA","nFeature_RNA","cell.complexity",
                           "percent.hb","percent.ribo.protein","percent.mt"))
v4
```

```{r save_group1_annotated_marker_violins, echo=FALSE, eval=FALSE}
pdf(paste0(out, "markers/auto_top3_markers_violin_annotated.pdf"), width = 6, height = 10)
v1
dev.off()

pdf(paste0(out, "markers/sandro_markers_violin_annotated.pdf"), width = 6, height = 10)
v2
dev.off()

pdf(paste0(out, "markers/kennedi_markers_violin_annotated.pdf"), width = 6, height = 8)
v3
dev.off()

pdf(paste0(out, "clustering_QC/qc_metrics_violin_annotated.pdf"), width = 6, height = 6)
v4
dev.off()

remove(v1,v2,v3,v4,markers,top5,top10)
```
...
## Cells to remove
```{r cells_to_remove_group1, eval=FALSE}
barcodes <- group1.annotated@meta.data
barcodes <- barcodes[group1.annotated$annotated_clusters %in% c("Unknown"),]
barcodes <- rownames(barcodes)
saveRDS(barcodes, "../../rObjects/pass1_group1_cells_to_remove.rds")
```

## Cellbender barcodes
```{r cellbender_barcodes}
cellbender_keep <- readRDS("../../rObjects/cellbender_kept_barcodes.rds")
group1.annotated$cellbender <- colnames(group1.annotated) %in% cellbender_keep
group1.annotated$cellbender <- gsub(TRUE, "keep", group1.annotated$cellbender)
group1.annotated$cellbender <- gsub(FALSE, "remove", group1.annotated$cellbender)
```

## Shiny app
```{r shiny_app_group1, eval=FALSE}
# create new object
...
# Recluster group 2
## Subset and recluster
Unknown2 clusters with B cells, DCs,  ILCs, Macrophages 1, Macrophages 2, Mast cells, Myeloid precursors, Neutrophils, T cells
```{r group2_subset_recluster}
# change out dir
remove(group1.annotated)
out <- "../../results/pass_1/recluster_group_2/"
# subset to group 2
mouse.annotated$OG_unannotated_clusters <- mouse.annotated$seurat_clusters
mouse.annotated$OG_annotated_clusters <- mouse.annotated$annotated_clusters
Idents(mouse.annotated) <- "annotated_clusters"
myClusters <- c("Unknown 2", "B cells", "DCs", "ILCs", "Macrophages 1",
                "Macrophages 2", "Mast cells", "Myeloid precursors", "Neutrophils",
                "T cells")
keepCells <- mouse.annotated@meta.data[,"annotated_clusters"] %in% myClusters
keepCol <- c(4:14,24,25)
meta <- mouse.annotated@meta.data[keepCells,keepCol]
...
group2 <- CreateSeuratObject(counts, 
                             ...
                             meta.data = meta)
remove(mouse.anotated,meta,counts)
...
group2 <- FindClusters(object = group2,
                       algorithm = 1, # 1 = Louvain
                       resolution = seq(0.1, 0.5,by = 0.1))
group2$seurat_clusters <- group2$SCT_snn_res.0.3
...
cluster_colors <- c("red4","red","orange","yellow","green","forestgreen",
                    "cyan","steelblue","blue","pink","deeppink","purple",
...
v3 <- VlnPlot(group2,
              ...
              flip = TRUE,
              cols = cluster_colors,
              features = c("C1qb","C1qa","Itgax","Itgam","nCount_RNA", "nFeature_RNA",
                           "cell.complexity","percent.mt", "percent.hb"))
v3
```

```{r save_group2_marker_violins}
pdf(paste0(out, "markers/auto_top3_markers_violin_unannotated.pdf"), width = 6, height = 10)
v1
dev.off()

pdf(paste0(out, "markers/sandro_markers_violin_unannotated.pdf"), width = 10, height = 10)
v2
dev.off()

pdf(paste0(out, "clustering_QC/qc_metrics_unannotated.pdf"), width = 8, height = 6)
v3
dev.off()
```
...
pseudo <- RunPCA(pseudo, 
                 features = VariableFeatures(object = pseudo))

# Step 6: Visualize PCA
pseudo$seurat_clusters <- factor(pseudo$seurat_clusters,
                                 levels = levels(group2$seurat_clusters))
DimPlot(pseudo, 
        reduction = "pca",
        group.by = "seurat_clusters",
        cols = cluster_colors,
...
group2.annotated <- RenameIdents(object = group2,
                                "0" = "Macrophages",         # Adap2
                                "1" = "Macrophages",         # Adap2
                                "2" = "Macrophages?Artifact?",     
                                "3" = "T and NK cells",      # Ms4a4b,Cd3e,Nkg7
                                "4" = "Neutrophils",         # Hp,Cxcr2   
                                "5" = "B cells",             # Cd19,Ms4a1  
                                "6" = "DCs",                 # Itgam,Itgax
                                "7" = "DCs",                 # Itgam,Itgax
                                "8" = "ILCs",                # Il7r,Il1rl1
                                "9" = "ILCs",                # Il7r,Il1rl1
                                "10" = "Mast cells",         # Kit
                                "11" = "Myeloid precursors", # Mki67
                                "12" = "B cells",            # Cecr2,Myb
                                "13" = "Plasma cells",       # Sdc1,Xbp1,Irf4
                                "14" = "Unknown")
...
# set colors
cluster_colors <- c("cyan","steelblue","blue","forestgreen","chartreuse","gold",
                    "red","pink","purple","gray","black")
...
## Cells to remove
```{r cells_to_remove_group2, eval=FALSE}
barcodes <- group2.annotated@meta.data
barcodes <- barcodes[group2.annotated$annotated_clusters %in% c("Unknown"),]
barcodes <- rownames(barcodes)
saveRDS(barcodes, "../../rObjects/pass1_group2_cells_to_remove.rds")
```

## Cellbender barcodes
```{r cellbender_group2}
cellbender_keep <- readRDS("../../rObjects/cellbender_kept_barcodes.rds")
group2.annotated$cellbender <- colnames(group2.annotated) %in% cellbender_keep
group2.annotated$cellbender <- gsub(TRUE, "keep", group2.annotated$cellbender)
group2.annotated$cellbender <- gsub(FALSE, "remove", group2.annotated$cellbender)
```

## Shiny app
```{r shiny_app_group2, eval=FALSE}
# create new object
library(ShinyCell)
...
# manual config edits
sc1conf <- readRDS("shinyApp/sc1conf.rds")
sc1conf[2,4] <- "cyan|steelblue|blue|forestgreen|chartreuse|gold|red|pink|purple|gray|black"
sc1conf[17,4] <- "red|gold|chartreuse|forestgreen|cyan|blue|pink|deeppink|purple|gray|black"
saveRDS(sc1conf, "shinyApp/sc1conf.rds")
```
