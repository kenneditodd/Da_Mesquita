...
- NicheNet can predict.. \
  - Which ligands from sender cell populations are most likely to affect target gene expression in receiver/target cell populations \
...
- Interested in the following combinations
  - 1) E4CM vs E3CM
    - Receiver: LECs
    - Sender: Senders Macrophages; B cells; T & NK cells; BECs
  - 2) Males DEGs in PLX E4 vs Control E4
    - Receiver: LECs
    - Sender: B cells; T & NK cells; BECs
  - 3) Females DEGs in Control E4 vs Control E3
    - Receiver: LECs
    - Senders: Macrophages; B cells; T & NK cells; BECs
  - 4) Females DEGs in PLX E4 vs Control E4
    - Receiver: LECs
    - Senders: B cells; T & NK cells; BECs
- Sender cells = cells that express ligands
- Reciever cells = cells that express the receptors
```{r nichenet_user_input}
# user defined variables
comparison <- "E4CM_vs_E3CM"
myReceiver <- "LECs"
mySender <- "Macrophages"
```

## Sender-agnostic potential ligands
First, we will do the sender-agnostic approach. If a receptor is expressed in a reciever cell population, all associated ligands will be used. This approach utilizes a ligand-receptor (LR) database.
```{r sender_agnostic_approach}
# determine all genes expressed in target cell population
cell_types <- levels(mouse.downsample$annotated_clusters)
all_genes_receiver <- get_expressed_genes(myReceiver, mouse.downsample)
print(paste("Reciever cell population =", myReceiver))
print(paste(length(all_genes_receiver), "genes expressed in the reciever cell population."))

# define expressed receptors in the target cell population
all_receptors <- unique(lr_network$to)
print(paste(length(all_receptors), "receptors in the ligand-receptor database."))
all_receptors_receiver <- intersect(all_receptors, all_genes_receiver)
print(paste(length(all_receptors_receiver), "receptors expressed in reciever cell population."))

# define potential ligands with expressed receptors
potential_ligands_sender_agnostic <- lr_network %>% 
  filter(to %in% all_receptors_receiver) %>% 
  pull(from) %>% 
  unique()
print(paste("There are", length(potential_ligands_sender_agnostic), "potential ligands."))
```

## Sender-focused approach
Second, we will do the sender-focused approach. Ligands expressed in senders whose matching receptors are expressed in the reciever.
```{r sender_focused_approach}
# get all sender cell populations
sender_celltypes <- cell_types[!cell_types %in% myReceiver]
...
all_ligands_sender <- 
  ...
  unlist() %>% 
  unique()
print(paste(length(all_ligands_sender), "expressed ligands from sender cell types."))

# get intersection to find ligands to focus on
potential_ligands_focused <- intersect(potential_ligands_sender_agnostic, 
                                       all_expressed_ligands_sender)
...
ligand_activities <- predict_ligand_activities(geneset = degs,
                                               ...
                                               potential_ligands = potential_ligands_sender_agnostic)
...
ligand_receptor_links_df <- get_weighted_ligand_receptor_links(
  best_upstream_ligands, all_expressed_receptors_receiver,
